 ...
 * and imports them into the Bridgeport Navigator database.
 *
...
async function searchServices(query: string, location: string = 'Bridgeport, CT', distance: number = 10): Promise<Service211[]> {
  if (!API_KEY) {
...
async function importService(service: Service211): Promise<boolean> {
  ...
  await prisma.resource.create({
    data: {
      ...
      city: service.location?.city || 'Bridgeport',
